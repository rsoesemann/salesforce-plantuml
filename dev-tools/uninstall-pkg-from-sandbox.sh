...
#### 1. Attempt to uninstall the specified package from the target QA org.
echoStepMsg "Attempt to uninstall package $PACKAGE_VERSION_ID from the org aliased as $QA_ORG_ALIAS"
echo \
...
if [ $? -ne 0 ]; then
  echoErrorMsg "Package removal from $QA_ORG_ALIAS was not successful."
fi

#### 2. List all of the packages installed in the target subscriber org.
echoStepMsg "List all packages that remain in $QA_ORG_ALIAS"
echo "List of packages currently installed in $QA_ORG_ALIAS.\n"
