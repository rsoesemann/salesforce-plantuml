...
# 3. Validate the expected alias and connection to the QA Org by showing limits from that org.
echoStepMsg "Validate connection and available limits of the org with the $QA_ORG_ALIAS alias"
(cd $PROJECT_ROOT && exec sfdx force:org:display \
                                --targetusername $QA_ORG_ALIAS \
                                --loglevel error)
